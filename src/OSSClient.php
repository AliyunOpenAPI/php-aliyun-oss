...
use Aliyun\OSS\Model\ListPartsInfo;
...
use Aliyun\OSS\Result\BodyResult;
use Aliyun\OSS\Result\ExistResult;
use Aliyun\OSS\Result\GetCorsResult;
use Aliyun\OSS\Result\GetLifecycleResult;
use Aliyun\OSS\Result\GetLoggingResult;
use Aliyun\OSS\Result\GetRefererResult;
use Aliyun\OSS\Result\GetWebsiteResult;
use Aliyun\OSS\Result\HeaderResult;
use Aliyun\OSS\Result\InitiateMultipartUploadResult;
use Aliyun\OSS\Result\ListBucketsResult;
use Aliyun\OSS\Result\ListMultipartUploadResult;
use Aliyun\OSS\Result\ListObjectsResult;
use Aliyun\OSS\Result\ListPartsResult;
...
class OSSClient
{
    ...
    private function generateResourceUri($options)
    {
        ...
        // resource_uri + object
        if (isset( $options[self::OSS_OBJECT] ) && '/' !== $options[self::OSS_OBJECT]) {
            $resource_uri .= '/' . str_replace(array( '%2F', '%25' ), array(
                    '/',
                    '%'
    ...
    private function generateSignableResource($options)
    {
        ...
        if (isset( $options[self::OSS_OBJECT] ) && '/' !== $options[self::OSS_OBJECT]) {
            $signableResource .= '/' . str_replace(array( '%2F', '%25' ), array(
                    '/',
                    '%'
                ), rawurlencode($options[self::OSS_OBJECT]));
        }
